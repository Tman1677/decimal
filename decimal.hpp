...
std::ostream &operator<<(std::ostream &outstream, const decimal &object) {
	...
	if (object.power > 0) {
		...
		if (object.power + outstring.length() == 0) {
			...
			outstring.insert(
			    outstring.begin() + outstring.length() + object.power, '.');
		}
	}
	outstream << outstring;
	return outstream;
...
int64_t decimal::tensPow(int64_t significand, int8_t power) {
	if (power < 0) {
		std::cerr << "This method should only be used with positive tens "
		             "powers to prevent a loss of data\n";
	}
	while (power > 0) {
		significand *= 10;
		power--;
...
decimal operator+(const decimal &object1, const decimal &object2) {
	if (object1.power == object2.power) {
		return decimal(object1.significand + object2.significand,
		               object1.power);
	} else if (object1.power < object2.power) {
		// shifts the number with a bigger power up an appropriate amount of
		// digits
		return decimal(object1.significand +
		                   decimal::tensPow(object2.significand,
		                                    object2.power - object1.power),
		               object1.power);
	} else {
		// shifts the number with a bigger power up an appropriate amount of
		// digits
		return decimal(object2.significand +
		                   decimal::tensPow(object1.significand,
		                                    object1.power - object2.power),
		               object2.power);
	}
}
...
decimal operator+(int64_t primitive1, const decimal &object1) {
	if (object1.power == 0) {
		...
		// digits
		return decimal(object1.significand +
		                   decimal::tensPow(primitive1, -object1.power),
		               object1.power);
	} else {
		// shifts the number with a bigger power up an appropriate amount of
		// digits
		return decimal(primitive1 +
		                   decimal::tensPow(object1.significand, object1.power),
		               0);
	}
}
...
decimal &decimal::operator+=(const decimal &object1) {
	if (object1.power == power) {
		...
		// digits
		significand = object1.significand +
		              decimal::tensPow(significand, power - object1.power);
		power = object1.power;
	} else {
		// shifts the number with a bigger power up an appropriate amount of
		// digits
		significand = significand + decimal::tensPow(object1.significand,
		                                             object1.power - power);
	}
	reduce();
	return *this;
...
bool operator==(const decimal &object1, const decimal &object2) {
	return object1.significand == object2.significand &&
	       object1.power == object2.power;
}
